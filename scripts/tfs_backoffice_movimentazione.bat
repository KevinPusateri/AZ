...
if /I %1==TEST (
    .\\node_modules\\.bin\\cypress run --quiet --browser E:\browsers_for_testing\GoogleChromePortable\App\Chrome-bin\chrome.exe --spec cypress/e2e/backoffice/movimentazione/*.js --env currentEnv=%1,isAviva=%2 --reporter cypress-mochawesome-reporter --reporter-options "reportDir=cypress/reports/movimentazione, inlineAssets=true, charts=true, embeddedScreenshots=true, reportPageTitle='Movimentazione Sinistri'"
) else (
